...
class ProjectPage(BasePage):
    def __init__(self,page: Page):
        ...
        self.BACK_TO_WORKSPACE = self.page.get_by_test_id("project-workspace-link")
        ...
        self.MOVE_FUNNEL_MODAL = self.page.get_by_role(role="heading", name="Move funnel to another workspace")
    ...
    def goto_project_page(self):
        if self.page.title() == "Editor | involve.me":
            self.click(self.BACK_TO_WORKSPACE)
        self.page.goto(config.PROJECTS_URL)

    def wait_for_page_load(self):
        try:
            self.FUNNEL_BLOCK_LIST.first.wait_for(state="visible",timeout=600)
        except TimeoutError:
            self.CREATE_NEW_FUNNEL_SCRATCH_BTN.wait_for(state="visible")
    ...
    def create_new_workspace(self,workspace_name: str):
        ...
        # ws_list.nth(ws_list.count()-1).wait_for(state="visible")

    def create_funnels(self,workspace: str,funnel_list: List[str]):
        for fnl in funnel_list:
            self.go_to_workspace(workspace)
            self.create_new_funnel_from_scratch(fnl, "Answer-based Outcomes")
            self.click(self.BACK_TO_WORKSPACE)

    def go_to_workspace(self,workspace_name: str):
    ...
    def get_workspace_locator(self,name: str) -> Locator:
        locator = self.filter(self.WORKSPACE_LIST,name)
        return locator

    def delete_workspace_by_name(self,name: str):
        ws_list = self.WORKSPACE_LIST
        before_count = ws_list.count() - 1
        self.go_to_workspace(name)
        self.click(self.WORKSPACE_OPTIONS_BTN)
        self.click(self.WORKSPACE_DELETE_BTN)
        self.fill(self.page.get_by_placeholder(f"{name}",exact=True), name)
        self.click(self.WORKSPACE_CONFIRM_BTN)
    ...
    def get_workspace_count(self)-> int:
        self.WORKSPACE_LIST.last.wait_for(state="visible")
        workspace_count = self.WORKSPACE_LIST.count()
        return workspace_count

    def get_funnel_count_from_ws_list(self,workspace_name: str) -> int:
        workspace = self.filter(self.WORKSPACE_NAME_LIST, workspace_name).first
        count = self.get_inner_text(workspace.locator("+ span"))
        return int(count)

    def move_funnel_workspace_to(self, workspace: str):
        dropdown = self.FUNNEL_MOVE_TO_DROPDOWN
        self.click(dropdown)
        dropdown.select_option(label=workspace)
        self.click(self.MOVE_FUNNEL_BTN)

    def create_new_funnel_from_scratch(self, name: str, funnel_type: FunnelTypes):
        if self.CREATE_NEW_FUNNEL_MENU_BTN.is_visible():
        ...
        self.click(self.page.locator("div.cursor-pointer",
                                     has=self.page.get_by_role("heading", name=funnel_type)))
        self.START_EDITING_BTN.wait_for(state="visible")
        self.click(self.START_EDITING_BTN)

    def create_new_funnel_with_ai(self,description: str):
        if self.CREATE_NEW_FUNNEL_MENU_BTN.is_visible():
            self.click(self.CREATE_NEW_FUNNEL_MENU_BTN)
    ...
    def open_publish_new_page(self,funnel_name: str) -> PreviewPage:
        ...
        return PreviewPage(new_page)

    def funnel_dropdown_menu_option(self,funnel_name: str, option: FunnelMenuOption):
        funnel = self.filter(self.FUNNEL_BLOCK_LIST, funnel_name, exact=True).first
        ...
        self.click(opt)
        if option == "Move to workspace":
            self.MOVE_FUNNEL_MODAL.wait_for(state="visible")

    def funnel_sort_by(self, option:SortTypes):
        self.select_option(self.FUNNEL_SORT_MENU, option)

    def search_funnel(self,name: str):
    ...
    def delete_funnel(self,funnel_name: str):
